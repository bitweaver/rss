<?php
/**
 * $Header: /cvsroot/bitweaver/_bit_rss/rss_lib.php,v 1.1.1.1.2.2 2005/06/27 15:56:41 lsces Exp $
 *
 ...
 *
 * $Id: rss_lib.php,v 1.1.1.1.2.2 2005/06/27 15:56:41 lsces Exp $
 * @package rss
 */
...
class RSSLib extends BitBase {
	...
	function list_rss_modules($offset, $maxRecords, $sort_mode, $find) {
		...
		$query = "select * from `".BIT_DB_PREFIX."tiki_rss_modules` $mid order by ".$this->convert_sortmode($sort_mode);
	...
	function remove_rss_module($rss_id) {
		...
		if( is_numeric( $rss_id ) ) {
			...
			$result = $this->query($query,array($rss_id));
			$ret = true;
		}
		return $ret;
	}
	...
	function refresh_rss_module($rss_id) {
		...
		if ($info) {
			...
			$result = $this->query($query,array((string)$data,(int) $now, (int) $rss_id));
			return $data;
		} else {
			return false;
		}
	...
	function get_rss_show_title($rss_id) {
		...
		if( is_numeric( $rss_id ) ) {
			...
			$ret = $this->getOne($query,array($rss_id));
		}
		return $ret;
	}
	...
	function get_rss_show_pub_date($rss_id) {
		...
		if( is_numeric( $rss_id ) ) {
			...
			$show_pub_date = $this->getOne($query,array($rss_id));
			$ret = $show_pub_date;
		}
		return $ret;
	}
